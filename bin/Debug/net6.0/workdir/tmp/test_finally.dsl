...
var(spec[static, nothing ], const RayInfo, _664 ) = 
{
	0.0F.xxx ,
	0.0F.xxx ,
	0.0F,
...
func(spec[nothing ], float2, Cam_GetViewCoordFromUV )
params(var(spec[nothing ], float2, vUV ) )
{
	var(spec[nothing ], float2, vWindow ) = (vUV * 2.0 ) - 1.0F.xx ;
	vWindow.x  *= (iResolution.x  / iResolution.y  );
	return <- vWindow;
};
func(spec[nothing ], float3x3, Cam_GetWorldToCameraRotMatrix )
params(var(spec[nothing ], CameraState, cameraState ) )
{
	var(spec[nothing ], float3, vForward ) = normalize(cameraState.vTarget  - cameraState.vPos  );
	var(spec[nothing ], float3, vRight ) = normalize(cross(float3(0.0, 1.0, 0.0 ), vForward ) );
	var(spec[nothing ], float3, vUp ) = normalize(cross(vForward, vRight ) );
	return <- float3x3(float3(vRight ), float3(vUp ), float3(vForward ) );
};
func(spec[nothing ], void, Cam_GetCameraRay )
params(var(spec[nothing ], float2, vUV ), var(spec[nothing ], CameraState, cam ), var(spec[out, nothing ], float3, vRayOrigin ), var(spec[out, nothing ], float3, vRayDir ) )
{
	var(spec[nothing ], float2, vView ) = Cam_GetViewCoordFromUV(vUV );
	vRayOrigin = cam.vPos ;
	var(spec[nothing ], float, fPerspDist ) = 1.0 / tan(radians(cam.fFov  ) );
	vRayDir = normalize(mul(float3(vView, fPerspDist ), Cam_GetWorldToCameraRotMatrix(cam ) ) );
};
func(spec[nothing ], float, GetVignetting )
params(var(spec[nothing ], float2, vUV ), var(spec[nothing ], float, fScale ), var(spec[nothing ], float, fPower ), var(spec[nothing ], float, fStrength ) )
{
	var(spec[nothing ], float2, vOffset ) = ((vUV - 0.5F.xx  ) * 1.41421354 ) * fScale;
	var(spec[nothing ], float, fDist ) = max(0.0, 1.0 - length(vOffset ) );
	var(spec[nothing ], float, fShade ) = 1.0 - pow(fDist, fPower );
	fShade = 1.0 - (fShade * fStrength );
	return <- fShade;
};
func(spec[nothing ], void, RayInfo_Clear )
params(var(spec[nothing ], int, i ), var(spec[inout, nothing ], ArrRayInfo, arrRayInfo ) )
{
...
func(spec[nothing ], void, RayStack_Reset )
params(var(spec[inout, nothing ], ArrRayInfo, rayStack ) )
{
	var(spec[nothing ], bool, _br_flag_7 ) = false;
...
func(spec[nothing ], RayInfo, RayStack_Get )
params(var(spec[nothing ], int, i ), var(spec[nothing ], ArrRayInfo, rayStack ) )
{
...
func(spec[nothing ], void, Scene_Union )
params(var(spec[inout, nothing ], SceneResult, a ), var(spec[nothing ], SceneResult, b ) )
{
	if(b.fDist  < a.fDist  )
	{
		a = b;
...
func(spec[nothing ], float, GetDistanceWine )
params(var(spec[nothing ], float3, vPos ) )
{
	...
	var(spec[nothing ], float, fBowlDistance ) = (length(vSphPos ) - 0.600000024 ) + 0.00999999977;
	var(spec[nothing ], float3, vWaterNormal ) = float3(0.0, 1.0, 0.0 );
	vWaterNormal.x  = sin(iTime * 5.0 ) * 0.00999999977;
	vWaterNormal.z  = cos(iTime * 5.0 ) * 0.00999999977;
	vWaterNormal = normalize(vWaterNormal );
	var(spec[nothing ], float, fWaterLevel ) = dot(vLocalPos, vWaterNormal ) - 0.100000001;
	return <- max(fBowlDistance, fWaterLevel );
};
func(spec[nothing ], float, GetDistanceBowl )
params(var(spec[nothing ], float3, vPos ) )
{
	var(spec[nothing ], float2, vPos2 ) = float2(length(vPos.xz  ), vPos.y  );
	var(spec[nothing ], float2, vSphOrigin ) = float2(0.0, 0.730000019 );
	var(spec[nothing ], float2, vSphPos ) = vPos2 - vSphOrigin;
	var(spec[nothing ], float2, vClosest ) = vSphPos;
	if(vClosest.y  > 0.100000001 )
	{
		vClosest.y  = 0.100000001F;
	};
	if(vClosest.y  < -0.699999988 )
	{
		vClosest.y  = -0.699999988F;
	};
	var(spec[nothing ], float, r ) = sqrt(1.0 - (vClosest.y  * vClosest.y  ) );
	vClosest.x  = r;
	...
	return <- fBowlDistance - 0.0299999993;
};
func(spec[nothing ], float, SmoothMin )
params(var(spec[nothing ], float, a ), var(spec[nothing ], float, b ), var(spec[nothing ], float, k ) )
{
	var(spec[nothing ], float, h ) = clamp(0.5 + ((0.5 * (b - a ) ) / k ), 0.0, 1.0 );
	return <- lerp(b, a, h ) - ((k * h ) * (1.0 - h ) );
};
func(spec[nothing ], float, GetDistanceWineGlass )
params(var(spec[nothing ], float3, vPos ) )
{
	var(spec[nothing ], float2, vPos2 ) = float2(length(vPos.xz  ), vPos.y  );
	var(spec[nothing ], float2, vSphOrigin ) = float2(0.0, 2.0 );
	var(spec[nothing ], float2, vSphPos ) = vPos2 - vSphOrigin;
	var(spec[nothing ], float2, vClosest ) = vSphPos;
	if(vClosest.y  > 0.300000012 )
	{
		vClosest.y  = 0.300000012F;
	};
	vClosest = normalize(vClosest ) * 0.600000024;
	var(spec[nothing ], float, fBowlDistance ) = distance(vClosest, vSphPos ) - 0.0149999997;
	var(spec[nothing ], float2, vStemClosest ) = vPos2;
	vStemClosest.x  = 0.0F;
	vStemClosest.y  = clamp(vStemClosest.y , 0.0, 1.35000002 );
	var(spec[nothing ], float, fStemRadius ) = vStemClosest.y  - 0.5;
	fStemRadius = ((fStemRadius * fStemRadius ) * 0.0199999996 ) + 0.0299999993;
	var(spec[nothing ], float, fStemDistance ) = distance(vPos2, vStemClosest ) - fStemRadius;
	var(spec[nothing ], float2, norm ) = float2(0.371390671, 0.928476691 );
	var(spec[nothing ], float2, vBaseClosest ) = vPos2;
	var(spec[nothing ], float, fBaseDistance ) = dot(vPos2 - float2(0.0, 0.100000001 ), norm ) - 0.200000003;
	...
	var(spec[nothing ], float, param_2 ) = 0.200000003;
	var(spec[nothing ], float, fDistance ) = SmoothMin(param, param_1, 0.200000003 );
	var(spec[nothing ], float, param_3 ) = fDistance;
	var(spec[nothing ], float, param_4 ) = fBaseDistance;
	var(spec[nothing ], float, param_5 ) = 0.200000003;
	fDistance = SmoothMin(param_3, param_4, 0.200000003 );
	fDistance = max(fDistance, vSphPos.y  - 0.5 );
	return <- fDistance;
};
func(spec[nothing ], void, Scene_Trim )
params(var(spec[inout, nothing ], SceneResult, a ), var(spec[nothing ], SceneResult, b ) )
{
	if(a.fDist  < (- b.fDist  ) )
	{
...
func(spec[nothing ], SceneResult, Scene_GetDistance )
params(var(spec[nothing ], float3, vPos ), var(spec[nothing ], int, iInsideObject ) )
{
	var(spec[nothing ], float3, vWineGlassPos ) = float3(0.0, 0.0, -2.0 );
	var(spec[nothing ], float3, vBowlPos ) = float3(1.0, 0.0, 1.0 );
	var(spec[nothing ], SceneResult, result );
	result.fDist  = vPos.y ;
	result.vUVW  = vPos;
	result.iObjectId  = 0;
	var(spec[nothing ], float3, vSphere1Pos ) = vBowlPos + float3(0.400000006, 0.5, 0.200000003 );
	var(spec[nothing ], SceneResult, sphereResult1 );
	sphereResult1.vUVW  = vPos - vSphere1Pos;
	sphereResult1.fDist  = min(result.fDist , length(vPos - vSphere1Pos ) - 0.400000006 );
	sphereResult1.iObjectId  = 4;
	var(spec[nothing ], SceneResult, param ) = result;
	Scene_Union(param, sphereResult1 );
	result = param;
	var(spec[nothing ], float3, vSphere2Pos ) = float3(2.20000005, 0.5, -0.899999976 );
	...
	var(spec[nothing ], float, fRadius ) = 1.0;
	var(spec[nothing ], float, fHeight ) = 1.0;
	var(spec[nothing ], SceneResult, glassResult );
	glassResult.iObjectId  = 1;
	glassResult.fDist  = length(vPos - float3(-2.0, 1.0, 1.0 ) ) - 1.0;
...
func(spec[nothing ], SceneResult, Scene_Trace )
params(var(spec[nothing ], float3, vRayOrigin ), var(spec[nothing ], float3, vRayDir ), var(spec[nothing ], float, minDist ), var(spec[nothing ], float, maxDist ), var(spec[nothing ], int, iInsideObject ) )
{
	var(spec[nothing ], SceneResult, result );
	result.fDist  = 0.0F;
	...
	for([var(spec[nothing ], int, i ) = 0 ], [], [++ i ] )
	{
		if(! _br_flag_17 )
		{
			if(i < 96 )
			{
				...
				t += result.fDist ;
				if(abs(result.fDist  ) < 0.00100000005 )
				{
					_br_flag_17 = true;
				};
				if(! _br_flag_17 )
				{
					...
					if(_420 )
					{
						_427 = abs(result.fDist  ) > 0.100000001;
					}
					else
					{
						_427 = _420;
...
func(spec[nothing ], float4, Env_GetSkyColor )
params(var(spec[nothing ], float3, vViewPos ), var(spec[nothing ], float3, vViewDir ) )
{
	var(spec[nothing ], float4, vResult ) = float4(0.0, 0.0, 1.0, 1100.0 );
	var(spec[nothing ], float3, vEnvMap ) = bufferA_iChannel1.SampleLevel (_bufferA_iChannel1_sampler, vViewDir, 0.0 ).xyz ;
	vEnvMap *= vEnvMap;
	var(spec[nothing ], float, kEnvmapExposure ) = 0.999000012;
	var(spec[nothing ], float3, _1954 ) = - log2(1.0F.xxx  - (vEnvMap * 0.999000012 ) );
	vResult.x  = _1954.x ;
	vResult.y  = _1954.y ;
	vResult.z  = _1954.z ;
	return <- vResult;
};
func(spec[nothing ], float3, Scene_GetNormal )
params(var(spec[nothing ], float3, vPos ), var(spec[nothing ], int, iInsideObject ) )
{
	var(spec[nothing ], float3, _func_ret_val_19 );
	var(spec[nothing ], bool, _func_ret_flag_19 ) = false;
	var(spec[nothing ], float2, e ) = float2(-1.0, 1.0 );
	var(spec[nothing ], float3, vNormal ) = (((e.yxx  * Scene_GetDistance(vPos + (e.yxx  * 0.00100000005 ), iInsideObject ).fDist  ) + (e.xxy  * Scene_GetDistance(vPos + (e.xxy  * 0.00100000005 ), iInsideObject ).fDist  ) ) + (e.xyx  * Scene_GetDistance(vPos + (e.xyx  * 0.00100000005 ), iInsideObject ).fDist  ) ) + (e.yyy  * Scene_GetDistance(vPos + (e.yyy  * 0.00100000005 ), iInsideObject ).fDist  );
	if(dot(vNormal, vNormal ) < 9.99999974E-6 )
	{
		_func_ret_flag_19 = true;
		_func_ret_val_19 = float3(0.0, 1.0, 0.0 );
	};
	if(! _func_ret_flag_19 )
	{
		_func_ret_flag_19 = true;
		_func_ret_val_19 = normalize(vNormal );
	};
	return <- _func_ret_val_19;
};
func(spec[nothing ], SurfaceInfo, Scene_GetSurfaceInfo )
params(var(spec[nothing ], float3, vRayOrigin ), var(spec[nothing ], float3, vRayDir ), var(spec[nothing ], SceneResult, traceResult ), var(spec[nothing ], int, iInsideObject ) )
{
	...
	if(traceResult.iObjectId  == 0 )
	{
		...
		surfaceInfo.vAlbedo  = bufferA_iChannel2.SampleLevel (_bufferA_iChannel2_sampler, traceResult.vUVW .xz  * 0.5, 0.0 ).xyz ;
		surfaceInfo.vAlbedo  *= surfaceInfo.vAlbedo ;
		surfaceInfo.fSmoothness  = clamp(1.0 - ((surfaceInfo.vAlbedo .x  * surfaceInfo.vAlbedo .x  ) * 2.0 ), 0.0, 1.0 );
		surfaceInfo.vAlbedo  *= 0.25F;
		var(spec[nothing ], float, fDist ) = length(surfaceInfo.vPos .xz  );
		var(spec[nothing ], float, fCheckerAmount ) = clamp((1.0 - (fDist * 0.100000001 ) ) + (surfaceInfo.vAlbedo .x  * 0.5 ), 0.0, 1.0 );
		var(spec[nothing ], float, fChecker ) = step(frac((floor(traceResult.vUVW .x  ) + floor(traceResult.vUVW .z  ) ) * 0.5 ), 0.25 );
		...
		surfaceInfo.fSmoothness  = lerp(surfaceInfo.fSmoothness , 1.0, fCheckerAmount );
	};
	if(traceResult.iObjectId  == 4 )
	{
		var(spec[nothing ], float, fStripe ) = step(frac(dot(traceResult.vUVW  * 5.0, float3(1.0, 0.200000003, 0.400000006 ) ) ), 0.5 );
		if(fStripe > 0.0 )
		{
			surfaceInfo.vAlbedo  = float3(0.100000001, 0.0500000007, 1.0 );
		}
		else
		{
			surfaceInfo.vAlbedo  = float3(1.0, 0.0500000007, 0.100000001 );
		};
	};
	if(traceResult.iObjectId  == 1 )
	{
		surfaceInfo.vR0  = 0.0199999996F.xxx ;
		var(spec[nothing ], float3, vAlbedo ) = bufferA_iChannel3.SampleLevel (_bufferA_iChannel3_sampler, traceResult.vUVW .xy  * 2.0, 0.0 ).xyz ;
		vAlbedo *= vAlbedo;
		vAlbedo *= float3(1.0, 0.100000001, 0.5 );
		surfaceInfo.vAlbedo  = vAlbedo;
		surfaceInfo.fSmoothness  = clamp(1.0 - (surfaceInfo.vAlbedo .x  * 2.0 ), 0.0, 0.899999976 );
		surfaceInfo.fTransparency  = clamp((surfaceInfo.vPos .y  * 5.0 ) + (surfaceInfo.vAlbedo .y  * 3.0 ), 0.0, 1.0 );
		var(spec[nothing ], bool, _1469 ) = surfaceInfo.vPos .x  < 0.0;
		var(spec[nothing ], bool, _1476 );
		if(! _1469 )
		{
			_1476 = surfaceInfo.vPos .z  < 0.0;
		}
		else
		{
			_1476 = _1469;
		...
		if(_1476 )
		{
			...
			surfaceInfo.fTransparency  = 1.0 - (vAlbedo.y  * 0.200000003 );
		};
		surfaceInfo.fRefractiveIndex  = 1.5F;
		var(spec[nothing ], float, fLipStickDist ) = length(surfaceInfo.vPos  - float3(0.300000012, 3.0999999, -2.9000001 ) ) - 1.0;
		if(fLipStickDist < 0.0 )
		{
			surfaceInfo.fTransparency  = clamp((1.0 + (fLipStickDist * 2.0 ) ) - vAlbedo.x , 0.0, 1.0 );
			surfaceInfo.vAlbedo  = float3(0.5, 0.0, 0.0 ) * (1.0 - surfaceInfo.fTransparency  );
		};
	};
...
func(spec[nothing ], float, Scene_TraceShadow )
params(var(spec[nothing ], float3, vRayOrigin ), var(spec[nothing ], float3, vRayDir ), var(spec[nothing ], float, fMinDist ), var(spec[nothing ], float, fLightDist ) )
{
	var(spec[nothing ], float, res ) = 1.0;
	var(spec[nothing ], float, t ) = fMinDist;
	var(spec[nothing ], bool, _br_flag_22 ) = false;
	for([var(spec[nothing ], int, i ) = 0 ], [], [++ i ] )
	{
		if(! _br_flag_22 )
		{
			if(i < 16 )
			{
				...
				res = min(res, (8.0 * h ) / t );
				t += clamp(h, 0.0199999996, 0.100000001 );
				if((h < 9.99999974E-5 ) || (t > fLightDist ) )
				{
					_br_flag_22 = true;
				};
			}
...
func(spec[nothing ], float, Light_GIV )
params(var(spec[nothing ], float, dotNV ), var(spec[nothing ], float, k ) )
{
	return <- 1.0 / (((dotNV + 9.99999974E-5 ) * (1.0 - k ) ) + k );
};
func(spec[nothing ], void, Light_Add )
params(var(spec[inout, nothing ], SurfaceLighting, lighting ), var(spec[nothing ], SurfaceInfo, surface ), var(spec[nothing ], float3, vViewDir ), var(spec[nothing ], float3, vLightDir ), var(spec[nothing ], float3, vLightColour ) )
{
	var(spec[nothing ], float, fNDotL ) = clamp(dot(vLightDir, surface.vBumpNormal  ), 0.0, 1.0 );
	lighting.vDiffuse  += (vLightColour * fNDotL );
	var(spec[nothing ], float3, vH ) = normalize((- vViewDir ) + vLightDir );
	var(spec[nothing ], float, fNdotV ) = clamp(dot(- vViewDir, surface.vBumpNormal  ), 0.0, 1.0 );
	var(spec[nothing ], float, fNdotH ) = clamp(dot(surface.vBumpNormal , vH ), 0.0, 1.0 );
	var(spec[nothing ], float, alpha ) = 1.0 - surface.fSmoothness ;
	var(spec[nothing ], float, alphaSqr ) = alpha * alpha;
	var(spec[nothing ], float, denom ) = ((fNdotH * fNdotH ) * (alphaSqr - 1.0 ) ) + 1.0;
	var(spec[nothing ], float, d ) = alphaSqr / ((3.14159274 * denom ) * denom );
	var(spec[nothing ], float, k ) = alpha / 2.0;
...
func(spec[nothing ], void, Light_AddDirectional )
params(var(spec[inout, nothing ], SurfaceLighting, lighting ), var(spec[nothing ], SurfaceInfo, surface ), var(spec[nothing ], float3, vViewDir ), var(spec[nothing ], float3, vLightDir ), var(spec[nothing ], float3, vLightColour ) )
{
	var(spec[nothing ], float, fAttenuation ) = 1.0;
	var(spec[nothing ], float, arg ) = 0.100000001;
	var(spec[nothing ], float, arg_1 ) = 10.0;
	var(spec[nothing ], float, fShadowFactor ) = Scene_TraceShadow(surface.vPos , vLightDir, 0.100000001, 10.0 );
	var(spec[nothing ], SurfaceLighting, param ) = lighting;
	var(spec[nothing ], SurfaceInfo, param_1 ) = surface;
	Light_Add(param, param_1, vViewDir, vLightDir, (vLightColour * fShadowFactor ) * 1.0 );
	lighting = param;
};
func(spec[nothing ], float, Scene_GetAmbientOcclusion )
params(var(spec[nothing ], float3, vPos ), var(spec[nothing ], float3, vDir ) )
{
	var(spec[nothing ], float, fOcclusion ) = 0.0;
	var(spec[nothing ], float, fScale ) = 1.0;
	var(spec[nothing ], bool, _br_flag_27 ) = false;
	for([var(spec[nothing ], int, i ) = 0 ], [], [++ i ] )
	{
		if(! _br_flag_27 )
		{
			if(i < 5 )
			{
				var(spec[nothing ], float, fOffsetDist ) = 0.00999999977 + ((1.0 * float(i ) ) / 4.0 );
...
func(spec[nothing ], SurfaceLighting, Scene_GetSurfaceLighting )
params(var(spec[nothing ], float3, vViewDir ), var(spec[nothing ], SurfaceInfo, surfaceInfo ) )
{
	...
	surfaceLighting.vDiffuse  += (g_vAmbientColor * (fAO * ((surfaceInfo.vBumpNormal .y  * 0.5 ) + 0.5 ) ) );
	return <- surfaceLighting;
};
func(spec[nothing ], float3, Light_GetFresnel )
params(var(spec[nothing ], float3, vView ), var(spec[nothing ], float3, vNormal ), var(spec[nothing ], float3, vR0 ), var(spec[nothing ], float, fGloss ) )
{
	var(spec[nothing ], float, NdotV ) = max(0.0, dot(vView, vNormal ) );
	return <- vR0 + (((1.0F.xxx  - vR0 ) * pow(1.0 - NdotV, 5.0 ) ) * pow(fGloss, 20.0 ) );
};
func(spec[nothing ], void, RayStack_Set )
params(var(spec[nothing ], int, i ), var(spec[nothing ], RayInfo, rayInfo ), var(spec[inout, nothing ], ArrRayInfo, rayStack ) )
{
...
func(spec[nothing ], float3, Env_ApplyAtmosphere )
params(var(spec[nothing ], float3, vColor ), var(spec[nothing ], float3, vRayOrigin ), var(spec[nothing ], float3, vRayDir ), var(spec[nothing ], float, fDist ), var(spec[nothing ], int, iInsideObject ) )
{
	...
	if((iInsideObject == 1 ) || (iInsideObject == 2 ) )
	{
		...
		if(iInsideObject == 2 )
		{
			vExtCol = float3(0.0, 0.5, 0.990000009 );
		}
		else
		{
			if(vRayOrigin.z  > 0.0 )
			{
				if(vRayOrigin.x  < 0.0 )
				{
					vExtCol = float3(0.990000009, 0.990000009, 0.0 );
				}
				else
				{
					vExtCol = float3(0.0, 0.800000011, 0.200000003 );
					vExtCol *= 20.0;
				};
			};
		};
...
func(spec[nothing ], float3, FX_Apply )
params(var(spec[nothing ], float3, vColor ), var(spec[nothing ], float3, vRayOrigin ), var(spec[nothing ], float3, vRayDir ), var(spec[nothing ], float, fDist ) )
{
	return <- vColor;
};
func(spec[nothing ], float4, Scene_GetColorAndDepth )
params(var(spec[nothing ], float3, vInRayOrigin ), var(spec[nothing ], float3, vInRayDir ) )
{
	...
	var(spec[nothing ], bool, _br_flag_34 ) = false;
	var(spec[nothing ], bool, _cont_flag_34 ) = false;
	for([var(spec[nothing ], int, iPassIndex ) = 0 ], [], [++ iPassIndex ] )
	{
		if(! _br_flag_34 )
		{
			_cont_flag_34 = false;
				...
				if(! _br_flag_34 )
				{
					...
					if(! _cont_flag_34 )
					{
						...
						else
						{
							...
							var(spec[nothing ], float, NdotV ) = clamp(dot(surfaceInfo.vBumpNormal , - rayInfo.vRayDir  ), 0.0, 1.0 );
							...
							if(surfaceInfo.fTransparency  > 0.0 )
							{
								...
								if(((vTestAmount.x  + vTestAmount.y  ) + vTestAmount.z  ) > 0.00999999977 )
								{
									...
									if(length(refractRayInfo.vRayDir  ) > 0.0 )
									{
										...
										refractRayInfo.fStartDist  = abs(0.100000001 / dot(refractRayInfo.vRayDir , surfaceInfo.vNormal  ) );
							...
							if(doReflection )
							{
								...
								if(((vTestAmount_1.x  + vTestAmount_1.y  ) + vTestAmount_1.z  ) > 0.00999999977 )
								{
									...
									reflectRayInfo.fStartDist  = abs(0.00999999977 / dot(reflectRayInfo.vRayDir , surfaceInfo.vNormal  ) );
	...
	var(spec[nothing ], bool, _br_flag_35 ) = false;
	var(spec[nothing ], bool, _cont_flag_35 ) = false;
	for([var(spec[nothing ], int, iStackPos ) = 11 ], [], [-- iStackPos ] )
	{
		if(! _br_flag_35 )
		{
			_cont_flag_35 = false;
			if(iStackPos >= 0 )
			{
				...
				if(rayInfo_1.fLengthRemaining  <= 0.0 )
				{
					_cont_flag_35 = true;
				};
				if(! _cont_flag_35 )
...
func(spec[nothing ], float, GetCoC )
params(var(spec[nothing ], float, fDistance ), var(spec[nothing ], float, fPlaneInFocus_1 ) )
{
	var(spec[nothing ], float, fAperture ) = 0.0500000007;
	var(spec[nothing ], float, fFocalLength ) = 0.800000011;
	return <- abs((0.0500000007 * (0.800000011 * (fDistance - fPlaneInFocus_1 ) ) ) / (fDistance * (fPlaneInFocus_1 - 0.800000011 ) ) );
};
func(spec[nothing ], float4, MainCommon )
params(var(spec[nothing ], float3, vRayOrigin ), var(spec[nothing ], float3, vRayDir ), var(spec[nothing ], float, fShade ) )
{
...
func(spec[nothing ], void, fillBufferA )
params(var(spec[out, nothing ], float4, vFragColor ), var(spec[nothing ], float2, vFragCoord ) )
{
	var(spec[nothing ], float2, vUV ) = vFragCoord / iResolution.xy ;
	var(spec[nothing ], float, fAngle ) = (iMouse.x  / iResolution.x  ) * 6.28318548;
	var(spec[nothing ], float, fElevation ) = (iMouse.y  / iResolution.y  ) * 1.57079637;
	if(iMouse.x  <= 0.0 )
	{
		fAngle = -2.29999995;
		fElevation = 0.5;
	};
	var(spec[nothing ], float, fDist ) = 6.0;
	var(spec[nothing ], CameraState, cam );
	cam.vPos  = float3((sin(fAngle ) * 6.0 ) * cos(fElevation ), sin(fElevation ) * 6.0, (cos(fAngle ) * 6.0 ) * cos(fElevation ) );
	cam.vTarget  = float3(0.0, 0.5, 0.0 );
	cam.fFov  = 20.0F;
	...
	var(spec[nothing ], float, param_2 ) = 0.699999988;
	var(spec[nothing ], float, param_3 ) = 2.0;
	var(spec[nothing ], float, param_4 ) = 1.0;
	var(spec[nothing ], float, fShade ) = GetVignetting(vUV, 0.699999988, 2.0, 1.0 );
...
func(spec[nothing ], float3, ColorGrade )
params(var(spec[inout, nothing ], float3, vColor ) )
{
	var(spec[nothing ], float3, vHue ) = float3(1.0, 0.699999988, 0.200000003 );
	var(spec[nothing ], float3, vGamma ) = 1.0F.xxx  + (vHue * 0.600000024 );
	var(spec[nothing ], float3, vGain ) = 0.899999976F.xxx  + ((vHue * vHue ) * 8.0 );
	vColor *= 2.0;
	var(spec[nothing ], float, fMaxLum ) = 100.0;
	vColor /= fMaxLum.xxx ;
...
func(spec[nothing ], float3, Tonemap )
params(var(spec[nothing ], float3, x ) )
{
	var(spec[nothing ], float, a ) = 0.00999999977;
	var(spec[nothing ], float, b ) = 0.131999999;
	var(spec[nothing ], float, c ) = 0.00999999977;
	var(spec[nothing ], float, d ) = 0.163000003;
	var(spec[nothing ], float, e ) = 0.101000004;
	return <- (x * ((x * 0.00999999977 ) + b.xxx  ) ) / ((x * ((x * 0.00999999977 ) + d.xxx  ) ) + e.xxx  );
};
...
params(var(spec[inout, nothing ], float4, fragColor ), var(spec[nothing ], float2, fragCoord ) )
{
	...
	var(spec[nothing ], float4, vSample ) = iChannel0.SampleLevel (_iChannel0_sampler, vUV, 0.0 );
	var(spec[nothing ], float, fCoC ) = vSample.w ;
	var(spec[nothing ], float3, vResult ) = vSample.xyz ;
	var(spec[nothing ], float, fTot ) = 0.0;
	var(spec[nothing ], float2, vangle ) = float2(0.0, fCoC );
	if(abs(fCoC ) > 0.0 )
	{
		vResult *= fCoC;
		fTot += fCoC;
		var(spec[nothing ], float, fBlurTaps ) = 32.0;
		var(spec[nothing ], bool, _br_flag_42 ) = false;
		for([var(spec[nothing ], int, i ) = 1 ], [], [++ i ] )
		{
			if(! _br_flag_42 )
			{
				if(i < 32 )
				{
					...
					var(spec[nothing ], float4, vTapSample ) = iChannel0.SampleLevel (_iChannel0_sampler, vTapUV, 0.0 );
					var(spec[nothing ], float, fCoC2 ) = vTapSample.w ;
					var(spec[nothing ], float, fWeight ) = max(0.00100000005, fCoC2 );
	...
	fragColor = float4(vResult, 1.0 );
	var(spec[nothing ], float, fExposure ) = 3.0;
	var(spec[nothing ], float4, _2327 ) = fragColor;
	var(spec[nothing ], float3, _2330 ) = _2327.xyz  * 3.0;
	fragColor.x  = _2330.x ;
	fragColor.y  = _2330.y ;
	fragColor.z  = _2330.z ;
...
params()
{
	g_vSunDir = float3(0.842151939, 0.336860776, 0.42107597 );
	g_vSunColor = float3(5.0, 3.5, 2.5 );
	g_vAmbientColor = float3(0.800000011, 0.200000003, 0.100000001 );
	fPlaneInFocus = 5.0;
	fGolden = 2.39996266;
};
