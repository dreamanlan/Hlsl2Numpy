...
func(spec[nothing], float3x3, lookat)params(var(spec[inout, nothing], float3, fw), var(spec[nothing], float3, up)) {
  fw = normalize(fw);
  var(spec[nothing], float3, rt) = normalize(cross(fw, normalize(up)));
  return <- float3x3(float3(rt), float3(cross(rt, fw)), float3(fw));
};

func(spec[nothing], float, hash)params(var(spec[nothing], float, n)) {
  return <- frac(sin(n) * 4378.54541F);
};

func(spec[nothing], float, noyz)params(var(spec[nothing], float3, x)) {
...
func(spec[nothing], float, fbm)params(var(spec[inout, nothing], float3, p)) {
...
func(spec[nothing], void, Kaleido)params(var(spec[inout, nothing], float2, v), var(spec[nothing], float, power)) {
  var(spec[nothing], float, a) = (floor(0.5F + ((atan2(v.x, -v.y) * power) / 6.28299999F)) * 6.28299999F) / power;
  v = (v * cos(a)) + (float2(v.y, -v.x) * sin(a));
};

func(spec[nothing], float, Rect)params(var(spec[nothing], float3, z), var(spec[nothing], float3, r)) {
  return <- max(abs(z.x) - r.x, max(abs(z.y) - r.y, abs(z.z) - r.z));
};

func(spec[nothing], float, DE)params(var(spec[inout, nothing], float3, z0), var(spec[inout, nothing], float4, mcol)) {
...
func(spec[nothing], float, CircleOfConfusion)params(var(spec[nothing], float, t)) {
  return <- max(abs(focalDistance - t) * aperture, pixelSize * (1.0F + t));
};

func(spec[nothing], float, linstep)params(var(spec[nothing], float, a), var(spec[nothing], float, b), var(spec[nothing], float, t)) {
  return <- clamp((t - a) / (b - a), 0.0F, 1.0F);
};

func(spec[nothing], float, randStep)params(var(spec[inout, nothing], float, randSeed)) {
  randSeed += 1.0F;
  return <- 0.800000011F + (0.200000003F * frac(sin(randSeed) * 4375.54541F));
};

func(spec[nothing], float, FuzzyShadow)params(var(spec[nothing], float3, ro), var(spec[nothing], float3, rd), var(spec[nothing], float, coneGrad), var(spec[nothing], float, rCoC), var(spec[inout, nothing], float4, mcol), var(spec[inout, nothing], float, randSeed)) {
  var(spec[nothing], float, t) = rCoC * 2.0F;
  var(spec[nothing], float, d) = 1.0F;
  var(spec[nothing], float, s) = 1.0F;
  for ([var(spec[nothing], int, i) = 0], [i < 6], [i++]) {
    if (s < 0.100000001F) {
      continue;
    };
...
func(spec[nothing], void, mainImage)params(var(spec[inout, nothing], float4, O), var(spec[nothing], float2, U)) {
  ...
  for ([var(spec[nothing], int, i) = 0], [i < 72], [i++]) {
