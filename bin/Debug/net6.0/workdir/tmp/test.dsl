...
func(spec[nothing], float3x3, fromEuler)params(var(spec[nothing], float3, ang)) {
...
func(spec[nothing], float, hash)params(var(spec[nothing], float2, p)) {
  var(spec[nothing], float, h) = dot(p, float2(127.099998F, 311.700012F));
  return <- frac(sin(h) * 43758.5469F);
};

func(spec[nothing], float, _noise)params(var(spec[nothing], float2, p)) {
...
func(spec[nothing], float, sea_octave)params(var(spec[inout, nothing], float2, uv), var(spec[nothing], float, choppy)) {
...
func(spec[nothing], float, map)params(var(spec[nothing], float3, p)) {
  ...
  for ([var(spec[nothing], int, i) = 0], [i < 3], [i++]) {
...
func(spec[nothing], float, heightMapTracing)params(var(spec[nothing], float3, ori), var(spec[nothing], float3, dir), var(spec[inout, nothing], float3, p)) {
  ...
  for ([var(spec[nothing], int, i) = 0], [i < 8], [i++]) {
...
func(spec[nothing], float, map_detailed)params(var(spec[nothing], float3, p)) {
  var(spec[nothing], float, freq) = 0.159999996F;
  var(spec[nothing], float, amp) = 0.600000024F;
  var(spec[nothing], float, choppy) = 4.0F;
  var(spec[nothing], float2, uv) = p.xz;
...
func(spec[nothing], float3, getNormal)params(var(spec[nothing], float3, p), var(spec[nothing], float, eps)) {
...
func(spec[nothing], float3, getSkyColor)params(var(spec[inout, nothing], float3, e)) {
...
func(spec[nothing], float, diffuse)params(var(spec[nothing], float3, n), var(spec[nothing], float3, l), var(spec[nothing], float, p)) {
  return <- pow((dot(n, l) * 0.400000006F) + 0.600000024F, p);
};

func(spec[nothing], float, specular)params(var(spec[nothing], float3, n), var(spec[nothing], float3, l), var(spec[nothing], float3, e), var(spec[nothing], float, s)) {
  var(spec[nothing], float, nrm) = (s + 8.0F) / 25.1327362F;
  return <- pow(max(dot(reflect(e, n), l), 0.0F), s) * nrm;
};

func(spec[nothing], float3, getSeaColor)params(var(spec[nothing], float3, p), var(spec[nothing], float3, n), var(spec[nothing], float3, l), var(spec[nothing], float3, eye), var(spec[nothing], float3, dist)) {
...
func(spec[nothing], float3, getPixel)params(var(spec[nothing], float2, coord), var(spec[nothing], float, time)) {
...
func(spec[nothing], void, mainImage)params(var(spec[inout, nothing], float4, fragColor), var(spec[nothing], float2, fragCoord)) {
  ...
  for ([var(spec[nothing], int, i) = -1], [i <= 1], [i++]) {
    for ([var(spec[nothing], int, j) = -1], [j <= 1], [j++]) {
