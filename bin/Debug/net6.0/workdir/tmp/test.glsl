...
layout (binding = 0) uniform sampler2D iChannel2;
layout (binding = 0) uniform sampler2D iChannel3;
layout (binding = 0) uniform sampler2D bufferA_iChannel0;
layout (binding = 0) uniform samplerCube bufferA_iChannel1;
layout (binding = 0) uniform sampler2D bufferA_iChannel2;
layout (binding = 0) uniform sampler2D bufferA_iChannel3;

#line 1 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
#line 19 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
vec4 LoadVec4( sampler2D samp, in vec2 vAddr )
{
    return texelFetch( samp, ivec2(vAddr), 0 );
}

vec3 LoadVec3( sampler2D samp, in vec2 vAddr )
{
    return LoadVec4( samp, vAddr ).xyz;
}
...
void Cam_LoadState( out CameraState cam, sampler2D samp, vec2 addr )
{
    vec4 vPos = LoadVec4( samp, addr + vec2(0,0) );
    cam.vPos = vPos.xyz;
    vec4 targetFov = LoadVec4( samp, addr + vec2(1,0) );
...
vec4 Scene_GetColorAndDepth( const in vec3 vInRayOrigin, const in vec3 vInRayDir )
{
 ...
 SceneResult firstTraceResult;
    ...
    rayStack.fLengthRemaining[0] = 1000.0;
...
#line 544 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
                    if ( length( refractRayInfo.vRayDir ) > 0.0 )
                    {
                        ...
                        stackEnd++;
                        stackEnd = clamp(stackEnd, 0, 12 - 1);
 ...
 for( int iStackPos=(12 -1); iStackPos >= 0; iStackPos-- )
 {
...
#line 669 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
SurfaceInfo Scene_GetSurfaceInfo( const in vec3 vRayOrigin, const in vec3 vRayDir, SceneResult traceResult, int iInsideObject )
{
...
#line 702 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
...
#line 740 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
...
vec4 Env_GetSkyColor( const vec3 vViewPos, const vec3 vViewDir )
{
 vec4 vResult = vec4( 0.0, 0.0, 1.0, 1100.0 );
...
#line 1067 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
    return vResult;
}
...
vec3 Env_ApplyAtmosphere( const in vec3 vColor, const in vec3 vRayOrigin, const in vec3 vRayDir, const in float fDist, const int iInsideObject )
{
    ...
    vec3 vResult = vColor;
#line 1094 "D:\\GitHub\\Hlsl2Numpy\\bin\\Debug\\net6.0\\workdir\\test.glsl"
