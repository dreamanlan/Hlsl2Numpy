...
func(spec[nothing], void, mainImage)params(var(spec[out, nothing], float4, fragColor), var(spec[in, nothing], float2, fragCoord)) {
  var(spec[nothing], float2, uv) = float2(556, 220) / iResolution.xy;
  uv = float2(0.72399999999999998, 0.50929999999999997);
  var(spec[nothing], float2, coord) = uv * iResolution.xy;
  coord = fragCoord;
  var(spec[nothing], float2, q) = coord.xy / iResolution.xy;
  ...
  ta.xz = mul(rot(((iTime + 285.0) * 0.050000000000000003 - floor((iTime + 285.0) * 0.050000000000000003 / 6.2831852000000001) * 6.2831852000000001)), ta.xz);
