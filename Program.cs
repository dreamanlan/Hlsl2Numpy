...
namespace Hlsl2Numpy
{
    internal partial class Program
    {
        ...
        {
            if (args.Length == 0) {
                Console.WriteLine("Usage:Hlsl2Numpy [-out outfile] [-args arg_dsl_file] [-notshadertoy] [-gl] [-profiling] [-torch] [-autodiff] [-gendsl] [-rewritedsl] [-printblocks] [-printfinalblocks] [-printgraph] [-printfinalgraph] [-printvecvars] [-unroll] [-novecterization] [-noconst] [-multiple] [-maxloop max_loop] [-debug] [-src ] hlsl_file");
                ...
                try {
                    ...
                    if (isDsl) {
                        ...
                        int r = -1;
                        if (OperatingSystem.IsWindows())
                            r = ProcessHelper.RunProcess("dxr.exe", "\"" + hlslFilePath + "\"", option, null, null, null, output, null, false, true, Encoding.UTF8);
                        else
                            r = ProcessHelper.RunProcess("dxr", "\"" + hlslFilePath + "\"", option, null, null, null, output, null, false, true, Encoding.UTF8);
                        if (r != 0) {
                    ...
                    if (dslFile.LoadFromString(dslTxt, msg => { Console.WriteLine(msg); })) {
                        ...
                        if (ProgramTransform.s_IsTorch) {
                            string incTxt = File.ReadAllText(Path.Combine(libDir, "hlsl_inc_torch.py"));
                            sb.AppendLine(incTxt);
                        }
                        else {
                            string incTxt = File.ReadAllText(Path.Combine(libDir, "hlsl_inc_numpy.py"));
                            sb.AppendLine(incTxt);
                        }
