...
{
    internal static class GlslFragInfo
    {
        internal static string s_GlslFragInVar = "inCoord";
        internal static string s_GlslFragOutVar = "outColor";
        internal static string s_GlslFragEntry = "frag_main";
    }

    internal partial class Program
    {
        private static void LoadShaderArgs(string argFilePath, string baseDir)
        {
                ...
                foreach (var cfg in cfgFile.DslInfos) {
                    ...
                    if (null != fd) {
                        if (id == "facecolor") {
                            ...
                            if (fd.IsHighOrder) {
                                ...
                                if (s_ShaderToyBufferNames.Contains(bufId)) {
                                    ...
                                    foreach (var bufArg in fd.Params) {
                                        ...
                                        if (null != bufArgFd) {
                                            SetShaderBufferInfo(bufInfo, true, bufArgFd, baseDir);
                                        }
                                    }
                                }
                            }
                            ...
                            SetShaderBufferInfo(s_MainShaderInfo, false, fd, baseDir);
                        }
                    }
                }
            }
        }
        private static void SetShaderBufferInfo(ShaderBufferInfo shaderInfo, bool isBuffer, Dsl.FunctionData bufArg, string baseDir)
        {
            ...
            if (argId == "entry") {
                ...
                SetShaderToyTex(shaderInfo, argId, bufArg.GetParamId(0), baseDir);
            }
        }
        private static void SetShaderToyTex(ShaderBufferInfo bufInfo, string key, string arg, string baseDir)
        {
            var texFilePath = arg;
            if (!s_ShaderToyBufferNames.Contains(arg)) {
                if (!File.Exists(Path.Combine(baseDir, texFilePath))) {
